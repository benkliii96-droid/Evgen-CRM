export function TableRow({ product, onEdit, onDelete, onImageClick }) {
  return (
    <tr className="border-b border-[#f0edf7] dark:border-[#3d3860] hover:bg-[#faf9fc] dark:hover:bg-[#2d2847] transition-colors">
      ...
      <td className="px-2 md:px-4 py-3 md:py-4 font-['Inter'] text-[12px] md:text-[14px] text-[#25213b] dark:text-white max-w-[100px] truncate">{product.category_name || product.category}</td>
      <td className="px-2 md:px-4 py-3 md:py-4 font-['Inter'] text-[12px] md:text-[14px] text-[#25213b] dark:text-white">{product.unit}</td>
      ...
      <td className="px-1 md:px-4 py-2 md:py-4">
        <div className="flex items-center">
          <button
            ...
            title="Редактировать"
          >
            <img src="/edit.svg" alt="Редактировать" className="w-[14px] h-[14px] md:w-[18px] md:h-[18px] dark:brightness-200" />
          </button>
          {product.image && (
            <button
            ...
            >
              <img src="/has_image.svg" alt="Фото" className="w-[14px] h-[14px] md:w-5 md:h-5 dark:brightness-200" />
            </button>
          )}
          <button
            ...
            title="Удалить"
          >
            <img src="/delete.svg" alt="Удалить" className="w-[14px] h-[14px] md:w-5 md:h-5 dark:brightness-200" />
          </button>
        </div>
      </td>
    </tr>
